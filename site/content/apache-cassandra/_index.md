...
annotations:
  ...
  summary: Unavailable exceptions have been encountered while performing writes in
    this cluster.
expr: |
  sum  without (cassandra_cluster) (cassandra_clientrequest_unavailables_count{clientrequest="Write"}) > 1
for: 5m
labels:
  severity: critical
{{< /code >}}
...
expr: |
  sum by (instance) (jvm_memory_usage_used_bytes{area="Heap"}) / sum by (instance) (jvm_physical_memory_size{}) * 100 > 80
for: 5m
labels:
  severity: critical
{{< /code >}}
