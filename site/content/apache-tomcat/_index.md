...
expr: |
  sum by (job,cluster,instance) (jvm_process_cpu_load{}) > 80
for: 5m
labels:
  severity: critical
{{< /code >}}
...
expr: |
  sum(jvm_memory_usage_used_bytes{}) by (job,cluster,instance) / sum(jvm_physical_memory_bytes{}) by (job,cluster,instance) * 100 > 80
for: 5m
labels:
  severity: critical
{{< /code >}}
...
expr: |
  sum by (job,cluster,instance) (increase(tomcat_errorcount_total{}[5m]) / increase(tomcat_requestcount_total{}[5m]) * 100) > 5
for: 5m
labels:
  severity: critical
{{< /code >}}
...
expr: |
  sum by (job,cluster,instance) (increase(tomcat_processingtime_total{}[5m]) / increase(tomcat_requestcount_total{}[5m])) > 300
for: 5m
labels:
  severity: warning
{{< /code >}}
