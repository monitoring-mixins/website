...
expr: "100 * sum by (instance, status) (rate(http_requests_total{status=~\"5[0-9][0-9]\"}[5m]))
...
expr: |
  100 * sum by (instance, status) (rate(http_requests_total{status=~"4[0-9][0-9]"}[5m])) / sum by (instance) (rate(http_requests_total{}[5m]))
  > 10
for: 5m
labels:
  severity: warning
