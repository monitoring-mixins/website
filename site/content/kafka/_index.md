...
expr: |
  sum by (job,kafka_cluster) (kafka_cluster_partition_underminisr{job="integrations/kafka"}) > 0
for: 5m
keep_firing_for: 5m
labels:
  severity: critical
...
##### KafkaPreferredReplicaImbalance
...
expr: |
  sum by (job,kafka_cluster) (kafka_controller_kafkacontroller_preferredreplicaimbalancecount{job="integrations/kafka"}) > 0
for: 30m
keep_firing_for: 5m
labels:
  severity: warning
