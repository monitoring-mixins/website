...
annotations:
  description: The CPU usage is at {{ printf "%.2f" $value }}% on {{$labels.core}}
    on {{$labels.host}} which is above the threshold of 80%.
...
annotations:
  description: The physical memory usage is at {{ printf "%.2f" $value }}% on {{$labels.host}}
    which is above the threshold of 80%.
  summary: Current physical memory usage of the host is approaching capacity.
expr: |
  100 * sum by (job, sid, host)(hanadb_host_memory_resident_mb{}) / sum by (job, sid, host) (hanadb_host_memory_physical_total_mb{}) > 80
for: 5m
labels:
  severity: critical
{{< /code >}}
...
annotations:
  description: The memory allocation limit is set at {{ printf "%.2f" $value }}% on
    {{$labels.host}} which is below the recommended value of 90%.
  summary: Memory allocation limit set below recommended limit.
expr: |
  100 * sum by (job, sid, host) (hanadb_host_memory_alloc_limit_mb{}) / sum by (job, sid, host) (hanadb_host_memory_physical_total_mb{}) < 90
for: 5m
labels:
  severity: warning
{{< /code >}}
...
expr: |
  100 * sum by (job, sid, host) (hanadb_host_memory_used_total_mb{}) / sum by (job, sid, host) (hanadb_host_memory_alloc_limit_mb{}) > 80
for: 5m
labels:
  severity: critical
{{< /code >}}
...
annotations:
  description: The disk usage is at {{ printf "%.2f" $value }}% on {{$labels.host}}
    which is above the threshold of 80%.
  summary: SAP HANA disk is approaching capacity.
expr: |
  100 * sum by (job, sid, host) (hanadb_disk_total_used_size_mb{}) / sum by (job, sid, host) (hanadb_disk_total_size_mb{}) > 80
for: 5m
labels:
  severity: critical
{{< /code >}}
...
annotations:
  description: The average SQL execution time is at {{ printf "%.2f" $value }}ms on
    {{$labels.host}} which is above the threshold of 1000ms.
  summary: SAP HANA SQL average execution time is high.
expr: |
  avg without (database_name, port, service, sql_type) (hanadb_sql_service_elap_per_exec_avg_ms{}) > 1000
for: 5m
labels:
  severity: critical
{{< /code >}}
...
annotations:
  description: The average system replication log shipping delay is at {{ printf "%.2f"
    $value }}ms from primary site {{$labels.site_name}} to replica site {{$labels.secondary_site_name}}
    which is above the threshold of 1000ms.
  summary: SAP HANA system replication log shipping delay is high.
expr: |
  avg without (database_name, port, secondary_port, replication_mode) (hanadb_sr_ship_delay{}) > 1000
for: 5m
labels:
  severity: critical
{{< /code >}}
...
- [sap-hana-logs](https://github.com/monitoring-mixins/website/blob/master/assets/sap-hana/dashboards/sap-hana-logs.json)
